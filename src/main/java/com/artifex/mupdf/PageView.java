...
public abstract class PageView extends ViewGroup {
  ...
  public PageView(Context c, Point parentSize) {
    ...
    mUsingHardwareAcceleration = false;// Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB;
  }
  ...
  public void addHq() {
    ...
    if (viewArea.width() != mSize.x || viewArea.height() != mSize.y) {
      ...
      Bitmap bm =
          Bitmap.createBitmap(patchArea.width(), patchArea.height(), Bitmap.Config.ARGB_8888);

      mDrawPatch = new SafeAsyncTask<PatchInfo, Void, PatchInfo>() {
        @Override
        protected PatchInfo doInBackground(PatchInfo... v) {
          drawPage(v[0].bm, v[0].patchViewSize.x, v[0].patchViewSize.y, v[0].patchArea.left,
  ...
  public void blank(int page) {
    ...
    if (mBusyIndicator == null) {
      ...
      mBusyIndicator.setBackgroundResource(R.drawable.pdf_shape_busy_progress);
  ...
  protected abstract void drawPage(Bitmap bm, int sizeX, int sizeY, int patchX, int patchY,
